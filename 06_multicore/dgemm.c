...
void do_block(int n, int si, int sj, int sk, double* A, double* BT, double* C){
  for(size_t i = si; i < si+BLOCKSIZE; i+=2*UNROLL){
    for(size_t j = sj; j < sj+BLOCKSIZE; ++j){
      ...
      for(size_t k = sk; k < sk+BLOCKSIZE; k+=2 * UNROLL){
        
        for(int r = 0; r < UNROLL; ++r){
          a[r] = vld1q_f64(&A[i*n + k + r*2]);
          b[r] = vld1q_f64(&BT[j*n + k + r*2]);
        }
